...
def manage_skips():
    ...
    leo_skip_sub.bytestring = (
        [0x97, 0x5C] +  # Fade to black, wait for fade
        [0xD0, 0x99] + # Set event bit 0x99, Found the Espers at Gathering Place of the Espers
...
def manage_dances():
    ...
    backgrounds = [
        [0x00, 0x05, 0x06, 0x07, 0x25, 0x29, 0x30, 0x36], #Wind Song
        [0x01, 0x03, 0x0F, 0x21], #Forest Suite
        ...
        [0x09, 0x0A, 0x0C, 0x11, 0x22, 0x26, 0x28, 0x2A, 0x2C], #Dusk Requiem
        [0x12] ] #Snowman Jazz
    fout.seek(0x11F9AB)
    for i, terrain in enumerate(backgrounds):
        #fout.seek(0x11F9A0 + i)
        fout.write(chr(random.choice(terrain)))
    
    # Change some dance associations to make more sense
